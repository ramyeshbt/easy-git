...
| 26 | **Every `read` from `/dev/tty` must have a TTY guard** — use `if [ -t 0 ]; then read -r val </dev/tty; else read -r val; fi` in ALL interactive-input functions (`prompt_input`, `fuzzy_select`, `undo_last`, any future helpers). `confirm()` already does this — keep all reads consistent. | `/dev/tty` crashes in CI, VS Code integrated terminal, and piped input with "No such device or address" |
